...
#pragma omp parallel for // for loops are extremly paralleizable
    ...
    // this is the job of the OS, it will use load balancing to still try split it evenly but it isthe casde that some threads will handle way more load , uses round robin fashion
}
